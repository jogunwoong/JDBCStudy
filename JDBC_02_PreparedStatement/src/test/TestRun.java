...
public class TestRun {
	...
	public static void insertTest() {
		...
		//4,5) sql문 실행하고 결과받기 (결과 : 처리된 행수-int)
		String sql = "INSERT INTO TEST VALUES(2, '김인창', SYSDATE)";
		// => 완성된 형태의 sql문 작성 (이대로 developer에서 실행해도 정상동작 되는 형태)
		// sql문 문장끝에는 세미콜론이 없어야 함!
		
		int result = stmt.executeUpdate(sql);
		/*
		 * DML 실행시 : stmt.executeUpdate(sql):int
		 * SELECT 실행시 : stmt.executeQuery(sql):Resultset
		 */
		
		//6) 트랜잭션 처리(DML 실행 시 트랜잭션 처리)
		conn.setAutoCommit(false);	//자동 커밋 옵션 false로 설정(jdbc6버전 이후 auto commit 설정)
